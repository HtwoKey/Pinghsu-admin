  ...
  <div class="app-container">
    ...
    <el-table v-loading="listLoading" :data="list" border stripe fit highlight-current-row style="width: 100%">
      ...
      <el-table-column align="center" label="操作" width="130">
        <template slot-scope="scope">
          <el-tooltip class="item" effect="dark" content="查看/回复评论" placement="top-start">
...
import {list,delComment,changeStatus,replyComment} from '@/api/comment'
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        replyContent: [{required: true,type:'string',message: '请输入',trigger:'blur'}],
      },
    }
  },
  created() {
  ...
  methods: {
    ...
    edit(row){
      ...
      this.temp.replyContent = '回复@'+row.name+'：';
    ...
    reply(){
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          ...
          params.append("article",this.temp.aid);
